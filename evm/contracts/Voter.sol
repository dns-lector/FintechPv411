...
contract Voter {
    ...
    string[] public candidates;  // але для масивів ці геттери приймають індекс
                                 // для отримання всього масиву потрібний явний геттер
    struct VoterData {
        bool isVoted;
        bool isAllowed;
    }
    ...
    // mapping(address => bool) isVoted;  // mapping - "словник" ключ-значення, який 
    // неявно має всі можливі ключі - на запит неіснуючого ключа видається об'єкт
    // заповнений значеннями за замовчанням
    event Voted(uint);
    ...
    function vote(uint i) external {
        ...
        require(!voters[msg.sender].isVoted, "Address is already voted");
...
- анонімність - не зберігається інформація про вибір, іде накопичення
