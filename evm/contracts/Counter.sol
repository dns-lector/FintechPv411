...
contract Counter {                    // контракт є аналогом класу (ООП)
  ...
  event Increment(int8 by);           // Події - засіб інформування про зміни
                                      // у смарт-контракті
  function inc() public {             // 
  ...
  function decBy(int8 by) public {    
    require(by > 0, "decBy: increment should be positive");
  ...
  function getCount() public view returns(uint) {
    return x;   // Solidity розрізняє звернення "х" та "this.x"
                // "х" - звернення до змінної (стану)
